...
/**
 * C++基础库的create_sign_fs64签名算法的java版本
 *
 * @author wangjiayin
 */
public class CreateSignFs64 {

    private static final long UNSIGNED_INT = (long) Integer.MAX_VALUE * 2 + 1;

    private static int[] MOD_PRIME_LIST_1 = new int[] {
    ...
    private static int[] MOD_PRIME_LIST_2 = new int[] {
    ...
    private static long getSign1(byte[] bytes) {

        long itemp = 0;

        for (int i = 0; i < bytes.length; i++) {
            int x = 0x00ff & bytes[i];
            itemp = ((long) x * (long) MOD_PRIME_LIST_1[0xFF & i] + itemp);
            if (itemp > UNSIGNED_INT) {
                itemp &= UNSIGNED_INT;
            }
    ...
    private static long getSign2(byte[] bytes) {

        long itemp = 0;

        for (int i = 0; i < bytes.length; i++) {
            int x = 0x00ff & bytes[i];
            itemp = ((long) x * (long) MOD_PRIME_LIST_2[0xFF & i] + itemp);
            if (itemp > UNSIGNED_INT) {
                itemp &= UNSIGNED_INT;
            }
        }
    ...
    private static long createSignFS64(byte[] bytes) {
        long sign1 = 0;
        long sign2 = 0;
        if (bytes.length <= 4) {
            sign1 = byteArray2Long(ArrayUtils.add(bytes, bytes.length, new Byte("0")), ByteOrder.LITTLE_ENDIAN);
        } else if (bytes.length <= 8) {
            byte[] firstPart = ArrayUtils.subarray(bytes, 0, 4);
            sign1 = byteArray2Long(ArrayUtils.add(firstPart, firstPart.length, new Byte("0")),
                    ByteOrder.LITTLE_ENDIAN);
            byte[] secondPart = ArrayUtils.subarray(bytes, 4, bytes.length + 1);
            sign2 = byteArray2Long(ArrayUtils.add(secondPart, secondPart.length, new Byte("0")),
                    ByteOrder.LITTLE_ENDIAN);

        } else {
            sign1 = getSign1(bytes);
            sign2 = getSign2(bytes);
        }
     ...
     * 以指定的字节序将字节数组转换为long
     */
    public static long byteArray2Long(byte[] bytes, ByteOrder order) {
        byte[] newBytes = Arrays.copyOf(bytes, bytes.length);
        if (order == ByteOrder.LITTLE_ENDIAN) {
            ArrayUtils.reverse(newBytes);
        }
        return new BigInteger(newBytes).longValue();
    }

    /**
     * 使用方法:
     * String a = "奔驰";
     * System.out.println(a + "=" + createSign(a.getBytes("gbk")));
     *
     * @param bytes 输入的字节数组,中文string必须按gbk编码转换
     *
     * @return 64位数字编码
     */
    public static BigInteger createSign(byte[] bytes) {
        BigInteger result = null;
        long sign = createSignFS64(bytes);
        if (sign < 0) {
            result = BigInteger.valueOf(Long.MAX_VALUE).add(BigInteger.valueOf(sign))
                    .subtract(BigInteger.valueOf(Long.MIN_VALUE)).add(BigInteger.valueOf(1));
        } else {
            result = BigInteger.valueOf(sign);
        }
        return result;
    }
